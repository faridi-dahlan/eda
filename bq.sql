...
CREATE OR REPLACE EXTERNAL TABLE `[project].[dataset].external_netflix_titles_bq`
(
  show_id INT64,
  type STRING,
  title	STRING,
...
OPTIONS (
  format = 'CSV',
  uris = [uri path untuk gcs],
  skip_leading_rows = 1,
  max_bad_records = 10
);

--transformasi tabel dari tabel eksternal (lowercase dan datetype)
CREATE OR REPLACE TABLE `[project].[dataset].external_netflix_titles_bq1` AS
...
FROM `[project].[dataset].external_netflix_titles_bq`
WHERE date_added != '' AND date_added IS NOT NULL;

--menyeragamkan min dan season menjadi min
CREATE OR REPLACE TABLE `[project].[dataset].external_netflix_titles_bq2` AS
SELECT 
  bq1.*,
  CASE
...
FROM 
  `[project].[dataset].external_netflix_titles_bq1` AS bq1;
